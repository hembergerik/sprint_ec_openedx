...
class EADatabase():
    ...
    def get_neighbors(self):
        ...
        neighbors=[]
        for row in rows:
            ind = {'hostname':str(row['hostname']), 'port':int(row['port']),
                   'send_file_name':str(row['send_file_name'])}
    ...
    def update_individual(self, individual, fitness, table):
        ...
        EADatabase.check_tablename(table)
        c = self.connection.cursor()        
        query = "SELECT id FROM %s WHERE individual=? ORDER BY id ASC LIMIT 1" % table
...
def test_database():
    ...
    for ind in individuals:
        db.store_individual(json.dumps(ind['genome']), json.dumps(ind['fitness']), table)
...
def test_EAIsland(parameters):
    EAI = EAIsland(parameters)
    individuals = []
    ind = {
        'genome':["IFLEQ",["IFLEQ","0.3","TURN_LEFT","SENSE_R","SENSE_A"],["+","0.3","0.3"],["IFLEQ","SENSE_R","TURN_RIGHT","TURN_RIGHT","0.6"],["+","0.1","SENSE_A"]],
        'fitness':0
    }
    individuals.append(ind)
    ind = {
